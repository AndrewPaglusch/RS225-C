...
// NOTE: cannot grow allocator, must fit max usage, left value shifted to MiB (arbitrary value)
#if defined(_arch_dreamcast) || defined(__NDS__)
    malloc_stats();
...
void getPlayerNewVis(Client *c, int size, Packet *buf) {
    int index;
    int added_count = 0;
    rs2_error("[CLIENT DEBUG] getPlayerNewVis START - size:%d bytes (%d bits), bit_pos:%d, local_player pos:(%d,%d)\n", 
    ...
    while (buf->bit_pos + 10 < size * 8) {
        index = gbit(buf, 11);
        rs2_error("[CLIENT DEBUG]   Read player index:%d at bit_pos:%d\n", index, buf->bit_pos - 11);
        
        if (index == 2047) {
            rs2_error("[CLIENT DEBUG]   End marker (2047) reached\n");
            break;
        }

        if (!c->players[index]) {
            rs2_error("[CLIENT DEBUG]   Creating new player at index %d\n", index);
        ...
        rs2_error("[CLIENT DEBUG]   Player %d: dx=%d dz=%d jump=%d -> absolute pos:(%d,%d)\n", 
    ...
    rs2_error("[CLIENT DEBUG] getPlayerNewVis END - added %d players, switching to byte access. pos:%d, bit_pos:%d\n", 
...
int main(int argc, char **argv) {
    ...
    // to print argv on emscripten, print index to flush instead of just \n?
